...
Public Class FrmManualGeneration
    ...
    Private Sub LoadData()
        ...
        AddComboBoxColumn("Venue", FetchVenues()) ' New ComboBox for venues
    End Sub
    ...
    Private Function FetchVenues() As List(Of String)
        Dim sql As String = "SELECT venue_name FROM Venues"
        Dim dt As DataTable = Crud(sql, Nothing)
        Return dt.AsEnumerable().Select(Function(row) row.Field(Of String)("venue_name")).ToList()
    End Function
    ...
    Private Sub SaveData()
        For Each row As DataGridViewRow In dgwManual.Rows
            If Not row.IsNewRow Then
                ...
                Dim venue As String = row.Cells("Venue").Value.ToString()
                Dim scheduleDate As Date = GetNextValidDate(day)
                SaveSchedule(courseId, day, timeSlot, session, userId, venue, scheduleDate)
                MessageBox.Show("Scheduled Successful", "Successful", MessageBoxButtons.OK, MessageBoxIcon.Information)
            End If
        Next
    End Sub

    Private Function GetNextValidDate(day As String) As Date
        Dim startDate As Date = DateTime.Now.AddDays(7)
        Dim targetDayOfWeek As DayOfWeek
    ...
    Private Sub SaveSchedule(courseId As Integer, day As String, timeSlot As String, session As String, userId As String, venue As String, scheduleDate As Date)
        Dim sql As String = "INSERT INTO Schedule (course_id, day, time_slot, session, userid, venue_id, date) VALUES (@course_id, @day, @time_slot, @session, @userid, (SELECT id FROM Venues WHERE venue_name = @venue_name), @schedule_date)"
        Dim parameters As New List(Of SqlParameter) From {
            ...
            New SqlParameter("@venue_name", SqlDbType.VarChar) With {.Value = venue},
            New SqlParameter("@schedule_date", SqlDbType.Date) With {.Value = scheduleDate}
        }
        Crud(sql, parameters)
    End Sub
