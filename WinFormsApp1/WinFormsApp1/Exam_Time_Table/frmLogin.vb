...
Public Class frmLogin
    ...
    Public Sub Automatically_BackUp()
        Dim backupDir As String = "C:\DATAbck"
        ...
        Using con As New SqlConnection(cs)
            con.Open()
            Dim backupQuery As String = "BACKUP DATABASE DATA_PROCESSING TO DISK='" & backupFilePath & "' WITH INIT, STATS=10"
    ...
    Public Sub CheckAndInsertRegistration()
        If IsRegistrationTableEmpty() Then
            InsertRegistration()
        End If
    End Sub

    Private Function IsRegistrationTableEmpty() As Boolean
        Dim sql As String = "SELECT COUNT(*) FROM Registration"
    ...
    Private Sub InsertRegistration()
        Dim sql As String = "INSERT INTO [dbo].[Registration] (
            [UserID], [UserType], [Password], [Name], [ContactNo], [SSN], 
            [EmailID], [JoiningDate], [Active], [PayrollType], [CardNo], 
            [AllowAccess], [Quotation], [BillReprint], [CreditCustomer], 
            [SalesReturn], [SalesList], [lall], [pharname]
        ) VALUES (
            @UserID, @UserType, @Password, @Name, @ContactNo, @SSN, 
            @EmailID, @JoiningDate, @Active, @PayrollType, @CardNo, 
            @AllowAccess, @Quotation, @BillReprint, @CreditCustomer, 
        ...
        Using conn As New SqlConnection(cs)
            Using cmd As New SqlCommand(sql, conn)
                cmd.Parameters.AddWithValue("@UserID", "Supports")
                cmd.Parameters.AddWithValue("@UserType", "Admin")
                cmd.Parameters.AddWithValue("@Password", "MTIzNA==") ' 
                cmd.Parameters.AddWithValue("@Name", "support")
                cmd.Parameters.AddWithValue("@ContactNo", "077171307")
                cmd.Parameters.AddWithValue("@SSN", DBNull.Value)
                cmd.Parameters.AddWithValue("@EmailID", "support@havano.net")
                cmd.Parameters.AddWithValue("@JoiningDate", New DateTime(2024, 3, 16, 5, 27, 21, 760))
                cmd.Parameters.AddWithValue("@Active", "Yes")
                cmd.Parameters.AddWithValue("@PayrollType", "Hourly Basis")
                cmd.Parameters.AddWithValue("@CardNo", DBNull.Value)
                cmd.Parameters.AddWithValue("@AllowAccess", "Yes")
                cmd.Parameters.AddWithValue("@Quotation", "Yes")
                cmd.Parameters.AddWithValue("@BillReprint", "Yes")
                cmd.Parameters.AddWithValue("@CreditCustomer", "Yes")
                cmd.Parameters.AddWithValue("@SalesReturn", "Yes")
                cmd.Parameters.AddWithValue("@SalesList", "Yes")
                cmd.Parameters.AddWithValue("@lall", "True")
                cmd.Parameters.AddWithValue("@pharname", DBNull.Value)
