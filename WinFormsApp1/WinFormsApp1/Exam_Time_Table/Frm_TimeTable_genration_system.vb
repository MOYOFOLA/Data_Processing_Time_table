...
Public Class Frm_TimeTable_genration_system
    ...
    Private Sub AutoGenerateTimetable()
            ...
            For Each day As String In days
                For Each timeSlot As String In timeSlots
                    ...
                    If lecturer IsNot Nothing Then
                        ...
                        If venueIndex >= venues.Rows.Count Then
                            venueIndex = 0 ' Reset to the first venue if we've used all available venues
                        End If

                        Dim venueId As Integer = venues.Rows(venueIndex)("id")
                        venueIndex += 1
        ...
        MessageBox.Show("Timetable generated successfully.", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub
    ...
    Private Function GetAvailableLecturer(staff As DataTable, assignedLecturers As List(Of Integer), day As String, timeSlot As String, deptId As Integer, facultyId As Integer) As DataRow
        Dim filter As String = $"deptid = {deptId} AND facultyid = {facultyId}"

        If assignedLecturers.Count > 0 Then
            filter &= $" AND userid NOT IN ({String.Join(",", assignedLecturers)})"
        End If

        Dim availableLecturers As DataRow() = staff.Select(filter)
